...
@section('content')
	<div class="user-tab-content">
		...
		<div class="col-span-full">
			...
			<x-theme-h1
					class="text-2xl mr-2 uppercase">{{ \App\Models\Staff\Consultation::count() }} {{__('consultation(s)')}}
				<span class="text-base">
                    <a href="{{ route('admin.users.laravel-consultations.index', $selu) }}">
                        (<span>{{ __('Display all for') }} {{ $selu->name }} | </span>
                    </a>
                    <a class="bg-yellow-200 px-1"
					   href="{{ route('admin.users.laravel-consultations.index', [$selu, 'filter' => $todayDate]) }}">{{ \App\Models\Staff\Consultation::where('booked_on', $todayDate)->count() }}
			...
			@isset($consultations)
				...
				<div class="font-semibold flex flex-row space-x-2 w-full items-center my-2 pb-1 border-yellow-500 border-b-2 justify-between">
					<div class="flex flex-row">
						...
						<a href="{{ route('admin.users.laravel-consultations.index',  [$selu, 'filter='.$yesterdayDate]) }}">
						...
						<a href="{{ route('admin.users.laravel-consultations.index', [$selu, 'filter='.$todayDate]) }}">
                            <span class="uppercase inline-flex items-center px-2.5 py-0.5 rounded-md text-sm font-medium font-bold
                                @if(Request::get('filter') == $todayDate) bg-yellow-500 text-gray-700 @else bg-gray-100 text-gray-800 @endif">
                                {{ __('today') }}
                            </span>
						</a>
						<a href="{{ route('admin.users.laravel-consultations.index', [$selu, 'filter='.$tomorrowDate]) }}">
                            <span class="uppercase inline-flex items-center px-2.5 py-0.5 rounded-md text-sm font-medium font-bold
					...
					<div class="flex flex-row">
						<a href="{{ route('admin.users.laravel-consultations.create', $selu) }}" class="btn-primary"><i
				...
				<div class="col-span-1 p-0">
					@forelse($consultations as $consultation)
						<div>
							...
							@endif
							<a href="{{ route('admin.users.laravel-consultations.show', [$selu, $consultation]) }}">{{$consultation->user->name }}
								| {{ $consultation->agency->code ?? 'NOT ASSIGNED' }}
								| {{ $consultation->staff->name ?? 'NOT ASSIGNED' }} | {{ $consultation->booked_on }}
					...
					@unless($selu || $url = Str::contains(Request::fullUrl(), 'filter'))
						{{ $consultations->links()  }}
					@endunless
					@else
						<a class="btn-primary" href="{{route('admin.users.laravel-consultations.create', $selu)}}">
